...
import { ListExecutionsUseCase } from '@promptdeploy/core';
...
export function makeListExecutionsHandler(executionRepo: ExecutionRepository) {
  const useCase = new ListExecutionsUseCase(executionRepo);
  ...
  const handler = async (
    ...
    // 2. Validate path params (projectId)
    ...
    const queryResult = listExecutionsQuerySchema.safeParse(event.queryStringParameters ?? {});
