...
import { UpdateExecutionUseCase } from '@promptdeploy/core';
import type { ExecutionRepository } from '@promptdeploy/core';
import {
  updateExecutionSchema,
  executionPathParamsSchema,
  tenantHeaderSchema,
} from '../schemas/index.js';
import { success, domainErrorResponse, validationError } from '../lib/responses.js';

export function makeUpdateExecutionHandler(executionRepo: ExecutionRepository) {
  const useCase = new UpdateExecutionUseCase(executionRepo);
  ...
  const handler = async (
    ...
    const pathResult = executionPathParamsSchema.safeParse(event.pathParameters);
    if (!pathResult.success) {
      return validationError('Invalid execution ID', pathResult.error.flatten());
    }

    // 3. Validate body
    const bodyResult = updateExecutionSchema.safeParse(event.body);
    ...
    const result = await useCase.execute({
      tenantId,
      executionId: pathResult.data.executionId,
